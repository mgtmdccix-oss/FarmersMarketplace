...
async function getBudgetSummary(userId) {
  ...
  const monthlyBudget =
    userRows[0]?.monthly_budget != null
      ? Number(userRows[0].monthly_budget)
      : null;
  ...
  const remaining =
    monthlyBudget == null
      ? null
      : Number((monthlyBudget - spentThisMonth).toFixed(7));
  const percentUsed =
...
router.patch("/budget", auth, async (req, res) => {
  if (req.user.role !== "buyer")
    return err(res, 403, "Only buyers can set budgets", "forbidden");
  ...
  if (
    ...
    return err(
      res,
      400,
      "monthly_budget must be a positive number or null",
      "validation_error",
    );
  }

  const budget = raw == null ? null : Number(raw);
  await db.query("UPDATE users SET monthly_budget = $1 WHERE id = $2", [
    budget,
    req.user.id,
  ]);
